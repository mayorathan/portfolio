...
const Project = ({ title, imgSrc, desc, stack }) => {
  const SkillItem = skill => (
    <p
      key={skill}
      className="text-foreground font-sans rounded-lg text-center  border-foreground p-2 border-2">
  ...
  return (
    <div className="flex flex-col place-self-start pt-10 pl-4 pr-4">
      <h2 className="text-4xl text-foreground tracking-wide">{title}</h2>
      <span className="flex flex-row">
        ...
        <span className="flex flex-col w-2/3 m-8">
          <p className="text-cloud tracking-wide">{desc}</p>
